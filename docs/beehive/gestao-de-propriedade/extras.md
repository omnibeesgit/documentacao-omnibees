...
title: Extras
description: Este documento detalha a gestão de Extras, que são serviços ou condições adicionais configuráveis para reservas. Abrange a listagem, criação, edição e eliminação de extras, incluindo as suas configurações de valor, tipo de cobrança, multilinguismo, gestão de imagens e a sua associação a tarifas e pacotes.
slug: /beehive/gestao-de-propriedade/extras
---

# Extras

Extras são serviços ou condições adicionais que podem ser incluídos numa reserva fora do quarto. Por exemplo, café da manhã, meia pensão, uma massagem, um buquê de flores, etc.
...
![Lista de Extras](</assets/beehive/gestao-de-propriedade/extras/08.png> "Lista de Extras")
...
*   Nome *(texto livre)*
*   Regime Alimentar *(lista de seleção única)*
*   Valor (moeda) *(numérico)*
*   Editável por *(lista de seleção única)*
*   Estado *(lista de seleção única)*
...
O filtro **Valor** é do tipo numérico, para valores maiores que zero e com duas casas decimais.
...
:::info
As ações adicionais (representadas na UI por 3 pontos) devem ser visíveis apenas quando um item é selecionado e as ações adicionais são: "**Compartilhar**", "**URL do Motor de Reservas**" e "**Histórico de Alterações**". As ações "**Compartilhar**" e "**URL do Motor de Reservas**" apenas são mostradas se a permissão "**Extras**" tiver a ação "**Ligado**" marcada.
:::
...
Neste ecrã, é possível criar ou editar um extra. Todos os campos obrigatórios devem ser preenchidos para que se possa **salvar** com sucesso. Quando esta operação for concluída, o extra estará pronto para ser utilizado.

![Criar/Editar Extras](</assets/beehive/gestao-de-propriedade/extras/09.png> "Criar/Editar Extras")
...
Ao criar um novo extra, o estado padrão é o estado ativo.
...
:::info
O **E-mail de Notificação** deve incluir uma *tooltip* com a seguinte informação: "Serviço ou pessoa a ser notificada automaticamente no caso do extra ser incluído em alguma reserva feita através do motor de reservas." Estas informações de e-mail normalmente são usadas quando o extra é fornecido por uma empresa externa (por exemplo, *rent-a-car*).
:::

É possível carregar, eliminar ou fazer o *download* de 1 ou mais fotos.
...
**Extras Multi-Property** (Editável por = Multi-Property) podem ser consultados pelo utilizador sem necessidade de qualquer permissão especial para tal.
Ao aceder aos detalhes de um extra Multi-Property, todos os dados estarão bloqueados, não sendo possível editar.
O utilizador poderá consultar as traduções definidas para o **Nome** e **Descrição**, os valores definidos em outras moedas e as fotos.

![Extras Multi-Property bloqueados](</assets/beehive/gestao-de-propriedade/extras/010.png> "Extras Multi-Property bloqueados")

Quando o extra está a ser criado, a opção de Ações adicionais (representadas na UI por 3 pontos) não deve estar visível, apenas no modo de edição ou consulta.
...
Ao associar/remover uma tarifa/pacote a um extra, essa ação terá de ser refletida também em **Tarifas** no separador **Extras Incluídos** e/ou no separador de **Extras Adicionais**.
...
Seja o extra definido como incluído ou adicional, o sistema valida se esse extra tem valor definido para a moeda da tarifa que está a ser associada.

Todas as alterações feitas na aba **Tarifas e Pacotes** ficam registadas nos históricos de alterações de Extras e de Tarifas.

![Tarifas e Pacotes associados a Extra](</assets/beehive/gestao-de-propriedade/extras/Screenshot_367.png> "Tarifas e Pacotes associados a Extra")

## Eliminar Extras

É possível eliminar um ou vários Extras, mesmo que estejam associados a tarifas ou pacotes.

É possível eliminar um ou vários Extras numa mesma ação.
